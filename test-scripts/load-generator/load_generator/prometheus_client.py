...
class PrometheusClient:
    ...
    def get_record_consumed_rates(self, job: str, start_ts: float, end_ts: float) -> list[tuple[datetime, float]]:
        params = {
        ...
        for series in data['data']['result']:
            for point in series['values']:
                result.append((datetime.fromtimestamp(float(point[0])), float(point[1])))

        print(f"Prometheus query result for job {job} between {start_ts} and {end_ts}: {result}")
        return result
